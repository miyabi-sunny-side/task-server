...
/// Version 14 lets the control plane send finished work back to its branch: a
/// `rework` task names the `normal` task it works on and what sent it back. A
/// column pair only — nothing a database already holds is being reworked.
const SCHEMA_V14: &str = "\
BEGIN;
ALTER TABLE tasks ADD COLUMN rework_target_task_id TEXT REFERENCES tasks(id);
ALTER TABLE tasks ADD COLUMN rework_reason TEXT;
PRAGMA user_version = 14;
COMMIT;
";

/// Version 15 adds the haystack: one appended row per run of an agent, a rescue
/// note, or a librarian watermark. Nothing here is updated or deleted; the
/// only later write is the retention sweep that blanks the two tails. The
/// partial unique index is the idempotency key for a worker's resend.
...
CREATE UNIQUE INDEX runs_idempotency ON runs (claim_id, attempt, source)
  WHERE claim_id IS NOT NULL AND attempt IS NOT NULL;
CREATE INDEX runs_task ON runs (task_id, id);
PRAGMA user_version = 15;
COMMIT;
";

/// Version 16 records when a task was called off (`cancelled` / `dropped`), so
/// the closed screen can order it next to finished work and the retention
/// sweep can age it out. Existing called-off rows take their last update as
/// the moment; nothing else changes.
const SCHEMA_V16: &str = "\
BEGIN;
ALTER TABLE tasks ADD COLUMN closed_at TEXT;
UPDATE tasks SET closed_at = updated_at WHERE status IN ('cancelled', 'dropped');
PRAGMA user_version = 16;
COMMIT;
";

/// Version 17 records who put a task into `blocked`: `operator` (a person
/// parking it), `worker` (a report that could not finish), or `system` (the
/// control plane, e.g. a dependency called off). Only the latter two are
/// stuck; a parked task is a decision, not a jam. Rows already blocked are
/// taken as `worker`, the only path a report could have used.
const SCHEMA_V17: &str = "\
BEGIN;
ALTER TABLE tasks ADD COLUMN blocked_by TEXT;
UPDATE tasks SET blocked_by = 'worker' WHERE status = 'blocked';
PRAGMA user_version = 17;
COMMIT;
";

/// Version 18 gives the haystack its reading receipt: `read_at` is when a
/// reader said it was done with the row, `read_note` what it did. Both NULL on
/// every existing row — nothing has been read until a reader says so.
const SCHEMA_V18: &str = "\
BEGIN;
ALTER TABLE runs ADD COLUMN read_at TEXT;
ALTER TABLE runs ADD COLUMN read_note TEXT;
PRAGMA user_version = 18;
COMMIT;
";

/// Version 19 adds `summary`: the one or two sentences a person reads as the
/// completion report, kept apart from `verification`, the log. NULL on every
/// existing row — nobody has written one yet.
...
impl Db {
    ...
    /// `PRAGMA user_version`: the schema step this database is at.
    ///
    /// # Errors
    /// `Error::Db` when the pragma cannot be read.
    pub fn schema_version(&self) -> Result<i64, Error> {
        self.with_conn(|conn| Ok(conn.query_row("PRAGMA user_version", [], |row| row.get(0))?))
    }
...
mod tests {
    ...
    /// The columns versions 10, 11, 13, 14, 16 and 17 added. Every migration test
    /// below asks about an earlier step, and a column that arrived after it
    /// would otherwise show up as a difference in every row comparison.
    const LATER_COLUMNS: [&str; 9] = [
        "release_level",
        "release_task_id",
        "depends_on",
        "done_at",
        "rework_target_task_id",
        "rework_reason",
        "closed_at",
        "blocked_by",
        "summary",
    ...
    /// Version 17 adds `blocked_by` and takes every row already blocked as the
    /// worker's doing — the one path a report could have used — and nothing else.
    #[test]
    fn a_version_sixteen_database_marks_existing_blocked_rows_as_the_workers() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("sqlite.db");
        ...
        for batch in [
            SCHEMA_V4, SCHEMA_V5, SCHEMA_V6, SCHEMA_V7, SCHEMA_V8, SCHEMA_V9, SCHEMA_V10,
            SCHEMA_V11, SCHEMA_V12, SCHEMA_V13, SCHEMA_V14, SCHEMA_V15, SCHEMA_V16,
        ] {
            legacy.execute_batch(batch).unwrap();
        }
        legacy
            .execute_batch(
                "INSERT INTO tasks (id, title, status, kind, product_id, created_at, updated_at)
                 VALUES ('t-jam', 'stopped', 'blocked', 'normal', 'a/b', 'then', 'later');
                 INSERT INTO tasks (id, title, status, kind, product_id, created_at, updated_at)
                 VALUES ('t-open', 'going', 'ready', 'normal', 'a/b', 'then', 'later');",
            )
            .unwrap();
        let before: i64 = legacy
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .unwrap();
        assert_eq!(before, 16, "the fixture must start at version 16");
        drop(legacy);

        let blocked_by = |db: &Db, id: &str| -> Option<String> {
            db.with_conn(|conn| {
                Ok(
                    conn.query_row("SELECT blocked_by FROM tasks WHERE id = ?1", [id], |row| {
                        row.get(0)
                    })?,
                )
            })
    ...
    /// across with none, because nobody has written one yet.
    #[test]
    fn a_version_eighteen_database_gains_an_empty_summary_column() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("sqlite.db");
        ...
        legacy
            .execute_batch(
                "INSERT INTO tasks (id, title, status, kind, product_id, verification, created_at, updated_at)
                 VALUES ('t-1', 'first', 'done', 'normal', 'a/b', 'cargo test', 'early', 'early');",
            )
            .unwrap();
        let before: i64 = legacy
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .unwrap();
        assert_eq!(before, 18, "the fixture must start at version 18");
        ...
        db.with_conn(|conn| {
            ...
            let summary: Option<String> =
                conn.query_row("SELECT summary FROM tasks WHERE id = 't-1'", [], |row| {
                    row.get(0)
                })?;
            assert!(summary.is_none(), "nobody has written one yet");
            assert_eq!(
                task_row(conn, "t-1"),
                row_before,
    ...
    /// Version 18 adds the reading receipt to the haystack and nothing else:
    /// every existing run comes across unread, with its own fields untouched.
    #[test]
    fn a_version_seventeen_database_marks_every_existing_run_unread() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("sqlite.db");
        ...
        legacy
            .execute_batch(
                "INSERT INTO runs (at, source, task_id, note)
                 VALUES ('2026-09-01T00:00:00Z', 'rescue', 't-1', 'seen');",
            )
            .unwrap();
        let before: i64 = legacy
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .unwrap();
        assert_eq!(before, 17, "the fixture must start at version 17");
        drop(legacy);
        ...
        db.with_conn(|conn| {
            ...
            let (note, read_at, read_note): (String, Option<String>, Option<String>) = conn
                .query_row(
                    "SELECT note, read_at, read_note FROM runs WHERE id = 1",
                    [],
                    |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
                )?;
            assert_eq!(note, "seen", "the row's own fields stay");
            assert!(read_at.is_none(), "an existing row is unread");
            assert!(read_note.is_none());
            Ok(())
        })
        .unwrap();
    }
    ...
    /// across untouched and `runs` starts empty with its idempotency index.
    #[test]
    fn a_version_fourteen_database_gains_an_empty_haystack() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("sqlite.db");
        ...
        for batch in [
            SCHEMA_V4, SCHEMA_V5, SCHEMA_V6, SCHEMA_V7, SCHEMA_V8, SCHEMA_V9, SCHEMA_V10,
            SCHEMA_V11, SCHEMA_V12, SCHEMA_V13, SCHEMA_V14,
        ] {
            legacy.execute_batch(batch).unwrap();
        }
        legacy
            .execute_batch(
                "INSERT INTO tasks (id, title, status, kind, product_id, created_at, updated_at)
                 VALUES ('t-1', 'first', 'done', 'normal', 'a/b', 'early', 'early');",
            )
            .unwrap();
        let before: i64 = legacy
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .unwrap();
        assert_eq!(before, 14, "the fixture must start at version 14");
        let row_before = task_row(&legacy, "t-1");
        drop(legacy);
        ...
        db.with_conn(|conn| {
            ...
            let indexed: i64 = conn.query_row(
                "SELECT count(*) FROM sqlite_master WHERE type = 'index' AND name = 'runs_idempotency'",
                [],
                |row| row.get(0),
            )?;
            assert_eq!(indexed, 1, "the idempotency index exists");
            assert_eq!(task_row(conn, "t-1"), row_before, "task rows are untouched");
            Ok(())
        })
        .unwrap();
    }

    /// Version 14 adds the rework columns and nothing else: every row comes
    /// across with both empty, because nothing that already existed is being
    /// reworked.
    #[test]
    fn a_version_thirteen_database_gains_empty_rework_columns() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("sqlite.db");
        ...
        for batch in [
            SCHEMA_V4, SCHEMA_V5, SCHEMA_V6, SCHEMA_V7, SCHEMA_V8, SCHEMA_V9, SCHEMA_V10,
            SCHEMA_V11, SCHEMA_V12, SCHEMA_V13,
        ] {
            legacy.execute_batch(batch).unwrap();
        }
        legacy
        ...
        let before: i64 = legacy
            .query_row("PRAGMA user_version", [], |row| row.get(0))
            .unwrap();
        assert_eq!(before, 13, "the fixture must start at version 13");
        let row_before = task_row(&legacy, "t-1");
        drop(legacy);
        ...
        db.with_conn(|conn| {
            ...
            let rework: (Option<String>, Option<String>) = conn
                .query_row(
                    "SELECT rework_target_task_id, rework_reason FROM tasks WHERE id = 't-1'",
                    [],
                    |row| Ok((row.get(0)?, row.get(1)?)),
                )
                .unwrap();
            assert_eq!(
                rework,
                (None, None),
                "an existing row is not being reworked"
            );

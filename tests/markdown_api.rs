...
async fn http_without_identity_retirement_snapshot_and_run_receipts() {
    ...
    assert_eq!(
        request(app.clone(), "POST", "/api/merges", json!({}))
            .await
            .0,
        StatusCode::GONE
    );
    ...
    let (_, a) = request(app.clone(), "POST", "/worker/runs", run.clone()).await;
    let (_, b) = request(app.clone(), "POST", "/worker/runs", run).await;
    assert_eq!(a["id"], b["id"]);
    let path = format!("/api/runs/{}/read", a["id"]);
    let (code, r) = request(app.clone(), "POST", &path, json!({"note":"filed"})).await;
    assert_eq!(code, StatusCode::OK);
    assert_eq!(r["read_note"], "filed");
    let (_, snap) = request(app, "GET", "/worker/snapshot", json!({})).await;
...
async fn mcp_flat_crud_contract_over_json_rpc() {
    ...
    for (target, count) in [("sandbox", 1), ("homeserver", 0)] {
        ...
        if count > 0 {
            assert_eq!(tasks[0]["execution_target"], target);
        }
    }
    state
        .store
    ...
    let (code, original) =
        request(app.clone(), "GET", &format!("/api/runs/{id}"), json!(null)).await;
    assert_eq!(code, StatusCode::OK);
...
async fn list_shapes_summary_projection_and_haystack_cursor_contract() {
    ...
    for path in ["/api/tasks", "/api/done", "/api/closed", "/api/products"] {
        let (status, list) = request(app.clone(), "GET", path, json!({})).await;
    ...
    let (_, detail) = request(app.clone(), "GET", "/api/tasks/draft", json!({})).await;
    ...
    let (_, first) = request(app.clone(), "GET", "/api/runs?limit=1", json!({})).await;
    assert_eq!(first["next"], 1);
    let (_, last) = request(app.clone(), "GET", "/api/runs?since=1&limit=1", json!({})).await;
...
async fn explicit_product_mcp_updates_reach_http_and_worker_without_rescan() {
    ...
    for path in ["/api/products/stable/id", "/worker/products/stable%2Fid"] {
        let (status, actual) = request(app.clone(), "GET", path, json!(null)).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(&actual, expected);
    }
    let (_, list) = request(app.clone(), "GET", "/api/products", json!(null)).await;
    assert_eq!(list[0]["archived"], true);
    assert_eq!(list[0]["repository"], "https://example/canonical/repo");
    let (status, _) = request(app, "POST", "/api/products/rescan", json!({})).await;
...
async fn mcp_tool_schemas_reject_ignored_arguments() {
    ...
    rejects_arguments(
        ...
        vec![
            ...
            ("task_update", json!({"id":"a","execution_target":"both"})),
...
async fn browser_writes_preserve_validation_and_state_constraints() {
    ...
    let (code, _) = request(app.clone(), "DELETE", "/api/tasks/browser-write", json!({})).await;
    assert_eq!(code, StatusCode::CONFLICT);
    let (code, card) = request(
        app.clone(),
        "POST",
...
async fn execution_targets_route_legacy_and_dependent_tasks() {
    ...
    state
        ...
        .put(
            "tasks",
            "dev",
            json!({"id":"dev","title":"legacy","status":"ready","product_id":"a/b"}),
        )
        .unwrap();
    ...
    request(
        ...
        "POST",
        "/worker/report",
        json!({"claim_id":dev["claim_id"],"outcome":"done"}),
    )
    .await;
    assert_eq!(
        request(
    ...
    for bad in [
        ...
        for (method, path, payload) in [
            (
                "POST",
                "/api/tasks",
                json!({"title":"bad","execution_target":bad}),
            ),
            ("PATCH", "/api/tasks/dev", json!({"execution_target":bad})),
            (
                "POST",
                "/worker/claim",
                json!({"worker":"bad","execution_target":bad}),
            ),
        ] {
            assert_eq!(
                request(app.clone(), method, path, payload).await.0,
...
fn concurrent_executors_claim_only_their_own_target_once() {
    ...
    let results = std::thread::scope(|scope| {
        let handles: Vec<_> = ["sandbox", "homeserver", "sandbox", "homeserver"]
            ...
            .map(|target| {
                ...
                scope.spawn(move || {
                    barrier.wait();
                    let result = task::claim(&state, target, None, target).unwrap();
                    if let Some(claim) = &result {
                        assert_eq!(claim["task"]["execution_target"], target);
                    }
                    result
                })
            })
    ...
    assert_ne!(results[0]["task"]["id"], results[1]["task"]["id"]);
}
